...
package Kernel::Modules::AgentTicketNoteToLinkedTicket;
...
sub Run {
    ...
    $Param{NoteToTicket} = $ParamObject->GetParam( Param => 'NoteToTicket' ) || "";

    @{ $Param{LinkedTicketIDs} } = $ParamObject->GetArray( Param => 'LinkedTicketIDs' );

    my %ArticleContent = $ArticleObject->ArticleGet(
    ...
    $GetParam{NoteToTicket} = $ParamObject->GetParam( Param => 'NoteToTicket' );
    @{ $GetParam{LinkedTicketIDs} } = $ParamObject->GetArray( Param => 'LinkedTicketIDs' );
        ...
        )
    {
            ...
            )
        {
            ...
            if ( $Param{LinkedTicketIDs} ) {
                for my $LinkedTicketID ( @{ $Param{LinkedTicketIDs} } ) {
                    my $LinkedArticleID
                    ...
                    if ( $Config->{LinkedTicketState} ) {
                        $TicketObject->TicketStateSet(
                            State     => $Config->{LinkedTicketStateDefault},
                            TicketID  => $LinkedTicketID,
        ...
        # if Article is given, get this article to generate
        # the quoted article content
        if ( $Param{ArticleID} ) {
        ...
        if ( $Config->{Subject} ) {
            $GetParam{Subject} = $Config->{Subject} . ' ' . $Param{ArticleContent}->{Subject};
        }
        else {
            $GetParam{Subject} = $Param{ArticleContent}->{Subject};
        }
        ...
        @{ $GetParam{Attachments} } = $UploadCacheObject->FormIDGetAllFilesMeta(
            FormID => $Self->{FormID},
        );
...
sub _Mask {
    ...
    if ( $Config->{Note} ) {
        ...
        if ( $Config->{NoteToTicket} ) {
            ...
            # build selection
            $Param{NoteToTicketStrg} = $LayoutObject->BuildSelection(
                Name       => 'NoteToTicket',
                Data       => \%YesNoList,
                SelectedID => $Param{NoteToTicket} // $Config->{NoteToTicketDefault},
        ...
        my $LinkedTickets = $Self->_GetLinkedTickets(%Param);

        # build selection
        $Param{LinkListStrg} = $LayoutObject->BuildSelection(
            Name       => 'LinkedTicketIDs',
            Data       => $LinkedTickets,
            SelectedID => $Param{LinkedTicketIDs},
            Sort       => 'AlphanumericValue',
            Multiple   => 1,
            Size       => 3,
            Class      => 'Validate_Required Modernize',
...
sub _GetLinkedTickets {
    ...
    # fill up hash for BuildSelection()
    return if !%LinkList;

    KEY:
    for my $Key ( sort keys %LinkList ) {
        if ( $LinkList{$Key}->{StateType} ne 'merged' ) {
            $ResultList{$Key} = $LinkList{$Key}->{TicketNumber} . ' - ' . $LinkList{$Key}->{Title};
            next KEY;
        }
        ...
        my @HistoryLines = $TicketObject->HistoryGet(
            TicketID => $LinkList{$Key}->{TicketID},
            UserID   => $Self->{UserID},
        );

        my $MergedTicketID;
        for my $HistoryLine (@HistoryLines) {
            my %HistoryData = %{$HistoryLine};

            if ( $HistoryData{HistoryType} eq 'Merged' ) {
                $HistoryData{Name} =~ /to \(\d*\/(\d{1,})\)/;
                $MergedTicketID = $1;
            }
        }

        # use only the last "merged" result
        my %MergedTicketData = $TicketObject->TicketGet(
            TicketID => $MergedTicketID
        );
        $ResultList{$MergedTicketID} = $MergedTicketData{TicketNumber} . ' - ' . $MergedTicketData{Title};
    }
